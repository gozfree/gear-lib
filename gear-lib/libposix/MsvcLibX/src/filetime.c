...
*    2014-07-03 JFL Filetime2String: Output time with 祍 precision if possib. *
...
/* Generate a string with the local file time, in the ISO 8601 date/time format */
/* 2014-07-03 Output time with 祍 precision if possible */
char *Filetime2String(const FILETIME *pFT, char *pBuf, size_t nBufSize) {
  ...
  if (nBufSize >= 20) {
    ...
    if (nBufSize >= 27) {
      ...
      iFraction /= 10; /* Convert 100ns resolution to 1祍 resolution */
